...
class Image:
	...
	#이미지를 흑백으로 변환한 뒤 Threshold 값을 기준으로 0 또는 1로 값을 정한다
        ...
        if self.contours:
            ...
            cv2.circle(self.image, (self.contourCenterX, self.middleY), 7, (255,255,0), -1) #Draw dX circle WHITE
            cv2.circle(self.image, (self.middleX, self.middleY), 3, (0,0,255), -1) #Draw middle circle RED
            
            font = cv2.FONT_HERSHEY_SIMPLEX
            print(str(self.middleX-self.contourCenterX))
            print("Weight:%.3f"%self.getContourExtent(self.MainContour))
            ...
            cv2.imshow("vision", self.image)
        return [self.contourCenterX, self.middleY], self.middleX-self.contourCenterX
